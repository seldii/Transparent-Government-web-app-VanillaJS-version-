...
//Change the key values inside the statistic object
        ...
        statistic.NumberOfDemocrats = JSON.stringify(democratsArr.length);
        statistic.NumberOfRepublicans = JSON.stringify(republicanArr.length);
        statistic.NumberOfIndependents = JSON.stringify(independentArr.length);
     

//Average vote w/ party

var votesWparty = democratsArr.map(x => x.votes_with_party_pct);

var arrSum = arr => arr.reduce((a,b)=> a+b, 0);

var averageVoteWParty = arrSum(votesWparty) / democratsArr.length;

statistic.PercOfVotedWPrty = JSON.stringify(averageVoteWParty);
